---
title: "Modularity function"
output: pdf_document
---

```{r}
# The Louvain algorithm, an open source code that is part of the NetworkToolbox package, was modified.
# Here is the original code: https://rdrr.io/github/AlexChristensen/NetworkToolbox/src/R/desc.all.R
# Modifications of the cited function included the optional change of the modularity matrix as input and expanded the output parameters to calculate the modularity of the modules.
```
...
```{r}
louvain <- function (A, gamma, M0, mat)
{
    if(missing(gamma))
    {gamma<-1
    }else{gamma<-gamma}
    ...
    for(i in 1:n)
        for(j in 1:n)
        {mat[i,j]<-(rowSums(A)[i]*colSums(A)[j])/s}
    }else(mat<-mat)
